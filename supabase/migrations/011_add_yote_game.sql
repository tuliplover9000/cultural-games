-- Migration 011: Add yote (Yoté) to valid games
-- Recreates record_game_result with 'yote' added to the whitelist so that
-- signed-in players' Yoté wins/losses persist stats and award coins server-side.
...
DECLARE
  ...
  v_valid_games text[] := ARRAY[
    ...
    'xinjiang-fangqi','filipino-dama','yote'
  ];
  v_base_coins  int := 0;
  v_bet_coins   int := 0;
  v_total_coins int := 0;
