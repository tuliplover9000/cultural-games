-- Migration 012: Add senet (Senet) to valid games
-- Recreates record_game_result with 'senet' added to the whitelist so that
-- signed-in players' Senet wins/losses persist stats and award coins server-side.
...
DECLARE
  ...
  v_valid_games text[] := ARRAY[
    ...
    'xinjiang-fangqi','filipino-dama','yote','senet'
  ];
  v_base_coins  int := 0;
  v_bet_coins   int := 0;
  v_total_coins int := 0;
